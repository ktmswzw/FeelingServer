...
public class PicCloud {
     ...
     * @param bucket     `                     空间名
     */
    public PicCloud(int appid, String secret_id, String secret_key, String bucket, String sign) {
        m_appid = appid;
        m_secret_id = secret_id;
        m_secret_key = secret_key;
        m_bucket = bucket;
    ...
    public int Upload(MultipartFile file, UploadResult result) {
        String fileid = "";
        ...
        try {
            ...
            String filename = file.getOriginalFilename();
                ...
                DataInputStream ins = new DataInputStream(file.getInputStream());
                int bytes = 0;
                byte[] bufferOut = new byte[1024];
                while ((bytes = ins.read(bufferOut)) != -1) {
                    out.write(bufferOut, 0, bytes);
    ...
    public int UploadGroup(InputStream fileStream, UploadResult result) {
        String fileid = "";
        PicAnalyze flag = new PicAnalyze();
        String req_url = GetUrl("0", fileid);
        String BOUNDARY = "---------------------------abcdefg1234567";
        ...
        try {
            ...
            DataInputStream ins = new DataInputStream(fileStream);
            int bytes = 0;
            byte[] bufferOut = new byte[1024];
            while ((bytes = ins.read(bufferOut)) != -1) {
                out.write(bufferOut, 0, bytes);
    ...
    public int Upload(byte[] file, UploadResult result) {
        String fileid = "";
        PicAnalyze flag = new PicAnalyze();
        String req_url = GetUrl("0", fileid);
        String BOUNDARY = "---------------------------abcdefg1234567";
